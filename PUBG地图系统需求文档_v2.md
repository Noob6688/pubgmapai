...
- **后端服务**: Supabase (PostgreSQL + PostGIS + Auth)
- **图标库**: Lucide React
- **类型语言**: TypeScript
- **对象存储**: Cloudflare R2
...
| F-005 | URL状态同步 | 筛选状态和当前地图同步到URL参数，便于分享和SEO | P1 |
...
- 新增类型：
  - 名称输入
  - 图标上传（支持PNG/SVG，存储到Cloudflare R2）
  - 颜色选择器
- 编辑类型：修改类型信息
- 删除类型：删除前检查是否有关联标记
- 排序调整：拖拽调整显示顺序
...
**注意**: 确保 Supabase 项目已启用 PostGIS 扩展：
```sql
...
create table marker_types (
  ...
  icon_url text,                         -- 图标URL（存储在Cloudflare R2）
...
create table markers (
  ...
  latlng geometry(Point, 3857),         -- PostGIS 坐标点 (lng, lat)
  title text,                            -- 标记标题
...
**坐标存储说明**: 使用 PostGIS 几何类型存储坐标，Supabase 自动处理坐标转换：
- 存储时：`ST_SetSRID(ST_MakePoint(lng, lat), 3857)` 或 `SRID=3857;POINT(lng lat)`
- 查询时：通过 `.select()` 获取后，通过 `latlng.coordinates` 访问 `[lng, lat]` 数组
...
create table recommendations (
  ...
  image_url text,                        -- 图片URL（存储在Cloudflare R2）
...
> **说明**: 以下移动端布局规范仅适用于地图查看页面，后台管理系统无需适配移动端。
...
**推荐**: Cloudflare Pages + @opennextjs/cloudflare
...
```json
{
  ...
  "devDependencies": {
    ...
    "@cloudflare/workers-types": "^4.20231101.0",
    "@types/leaflet": "^1.9.8",
    "@types/node": "^20.0.0",
    "@types/react": "^18.2.0",
    "typescript": "^5.0.0",
